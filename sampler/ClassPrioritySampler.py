...
class PriorityTree(object):
    def __init__(self, capacity, init_weights, fixed_weights=None, fixed_scale=1.0,
        ...
        """
        # fixed_weights = np.array([0.7, 0.3])
...
class ClassPrioritySampler(Sampler):
    ...
    ex: feat_shift.yaml
    {alpha: 1.0, cycle: 0, decay_gap: 30, def_file: ./data/ClassPrioritySampler.py,
    epochs: 90, fixed_scale: 1, lam: null, manual_only: true, momentum: 0.0, nroot: null,
    pri_mode: train, ptype: score, rescale: false, root_decay: null, type: ClassPrioritySampler}
    """
    def __init__(self, dataset, balance_scale=1.0, fixed_scale=1.0,
        ...
        labels = []
        for data_source_iter in dataset:
            labels.append(data_source_iter['labels'])
        self.labels = labels
        ...
        for i, cls_idx in enumerate(self.cls_idxs):
            self.cnts[i] = len(cls_idx)
