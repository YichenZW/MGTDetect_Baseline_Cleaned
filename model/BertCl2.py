...
#         print('self_train_id: ', self.train_idx_by_label)
    ...
    #         print('q_feat size: ', q_feat.size())
            ...
            contraloss = self.contrastive_loss_labelwise_winslide(batch_size, batch_idx_by_label, q_feat)
            clsloss = self.custom_loss(batch_size, ce_q_loss, q_value, labels)

            # momentum update model k
            self.momentum_update(m=0.999)

            loss = (1.0 - self.args.model.contraloss_weight) * clsloss + self.args.model.contraloss_weight * contraloss
        ...
        if flag[0] == 2:
            ...
            return {'logits': eval_logits, 'loss': eval_loss}
    ...
    def custom_loss(self, batch_size, ce_loss, logits, labels):
        if self.args.model.ce_loss:
            return ce_loss
        else:
            act = nn.Sigmoid()
            prob = act(logits)
#             print('prob: ', prob.size())
            focal_loss = -self.args.model.alpha * (1-prob) ** self.args.model.gamma * labels * torch.log(prob)- (1-self.args.model.alpha) * prob ** self.args.model.gamma * (1-labels) * torch.log(1-prob)
#             print('focal_loss: ', focal_loss.size(), focal_loss)
