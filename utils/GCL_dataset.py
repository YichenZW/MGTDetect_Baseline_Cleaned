...
class sentsplitter(SpacySentenceSplitter):
    def my_splitter(self, text: str):
        if self._is_version_3:
            return [sent.text.strip() for sent in self.spacy(text).sents if sent.text!='\n']
        else:
            return [sent.string.strip() for sent in self.spacy(text).sents if sent.text!='\n ']
...
class TokenizedDataset(Dataset):
    ...
    def __init__(self, args, training_args, tokenizer, raw_dataset,flags, sent_model, ):
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        ##TODO:graph construction and read graph
        ##TODO:output adj_matrix, sentences(add to node attribute)
        ...
        tokenized_input = self.tokenizer.batch_encode_plus(
            ...
            padding="max_length", 
            truncation=True,
            max_length=self.training_args.input_max_length,  #remember to be set to 32 or 64
...
#         batched_adj = adj[0]
#         for i in range(1, len(adj)-1, 1):
#             batched_adj = self._adjConcat(batched_adj, adj[i])

        return {'input_ids': torch.LongTensor(batched_input_ids),
